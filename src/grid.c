...
/* grid processor splitting */
  ...
  if(ROOT){
	...
	if(sort3d[2]==LNY){
	  ...
	  if(sort3d[2]==LNZ){ LNZ/=2; nzprocs*=2;}  
	}
      }
      if(LNX%2!=0 && LNY%2!=0 && LNZ%2!=0 )break;
    }
    if(nxprocs*nyprocs*nzprocs==nprocs){  if(!me)fprintf(stderr,"good splitting!\n");
    }else{
      if(!me)fprintf(stderr,"bad splitting: %d over %d\n", nxprocs*nyprocs*nzprocs , nprocs);
    }
    if(!me) fprintf(stderr,"LNX %d LNY %d LNZ %d\n",LNX,LNY,LNZ);
    if(!me) fprintf(stderr,"nxprocs %d nyprocs %d nzprocs %d total %d\n",nxprocs,nyprocs,nzprocs, nxprocs*nyprocs*nzprocs);
  }/* if ROOT */
...
/* flag: 1 is bulk , 0 is wall , -1 is dormient */  
...
void compute_volumes(){ 
 ...
 my_double D17[3],D35[3],D03[3],D12[3],D05[3],D14[3],D06[3],D24[3],D27[3],D36[3],D47[3],D56[3];
 int n;
 my_double S1357, S0145, S0246, S0123, S4567, S2367;
 my_double N1357[3], N0145[3], N0246[3], N0123[3], N4567[3], N2367[3];
...
/* diagonals definition of each surface of a polyhedron treating as a vector */
    for(n=0;n<3;n++){
      D17[n]= P7[n]-P1[n];
      D35[n]= P5[n]-P3[n];
      D03[n]= P3[n]-P0[n];
...
Vector formulas for a quadrilateral:
...
S = 0.5 * mod(AC X BD)
and the magnitude of AC X BD is calculated by finding the determinant of matrix formed
 */
...
Normal vector of a plane with known two vectors can be calculated by finding the cross product of these two vectors.
Here, AC X BD will give the normal vector to the plane where AC and BD lie.
Also, X[0] = i component
      X[1] = j component
      X[2] = k component

These vector are constructed in a way to have their norms equal to the surface S to which they are perpendicular
     e.g.  M = 0.5 ( AC X BD )
*/
    int x=0,y=0;
    for(n=0;n<3;n++){
...
/* volume definition 
Ref:  Jeffrey Grandy, Efficient Computation of Volume of Hexahedral Cells, Lawrence Livermore National Laboratory
*/
